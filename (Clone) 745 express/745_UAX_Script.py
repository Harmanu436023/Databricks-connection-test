...
os.chdir(r'D:\UDH_Migration\745_Express')
...
EDW_SQL_9= SQL_FOLDER +r'/Cancelled_flights_SQL_9.sql'  ## issue # fixed 4/10
EDW_SQL_10= SQL_FOLDER +r'/Yet_to_Depart_flights_SQL_10.sql'
EDW_SQL_11= SQL_FOLDER +r'/Residual_cancels_SQL_11.sql' ## issue # fixed 4/10
...
## done till here
DELAYS_SQL= SQL_FOLDER +r'/Delay_Info.sql'
...
EDW_SQL_17 = SQL_FOLDER + r'/UA_Seat_Cancels.sql'
EDW_SQL_18 = SQL_FOLDER + r'/UA_SeatCnxl_2019.sql'
...
UA_SeatCnxl_2019_CSV_FILE = CSV_FLDR + r'UA_SeatCnxl_2019' +TODAY+'.csv'
...
def send_error_email(attachment, extra_subject=''):
    email_list_to = ['harmanpreet.singh@united.com']

    sender_address = 'harmanpreet.singh@united.com'
...
connection_params = {
    ...
    "Server": "cbs-udh-datalake-prod-redshift-dw1.csjkuisl0oi7.us-east-1.redshift.amazonaws.com",
    "Database": "udhprod",
    "UID": udh_username,
    "PWD": udh_pass
}
connection = pyodbc.connect(";".join([f"{k}={v}" for k, v in connection_params.items()]))
cursor = connection.cursor()
logging.info('Checking SuperSnapshot data - yesterdays')
SS_CNT = pd.read_sql('select count(*) as cnt from co_prod_vmdb.rtf_flt_leg_operation_ss where report_dt = current_date-1', connection)
...
while SS_CNT <1000:
    SS_CNT = pd.read_sql(
        'select count(*) as cnt from co_prod_vmdb.rtf_flt_leg_operation_ss where report_dt = current_date-2', connection)
    SS_CNT = SS_CNT['cnt'][0]
    logging.info('Number of rows in SuperSnapsot: '+str(SS_CNT))
    if SS_CNT > 1000:
        ss_flag = True
...
if ss_flag == True:
    ...
    sqlEdw22 = e22.read()
    e22.close()
    e23 = open(EDW_SQL_17, 'r')
    sqlEdw23 = e23.read()
    e23.close()
    e24 = open(EDW_SQL_18, 'r')
    sqlEdw24 = e24.read()
    e24.close()
    ...
    sqlresult_8 = pd.read_sql_query(sqlEdw8, connection)
    sqlresult_8.to_csv(FLIGHTS_NOT_DEPARTED_SQL_8_CSV_FILE, index=False)
    sqlresult_9 = pd.read_sql_query(sqlEdw9, connection)
    sqlresult_9.to_csv(Cancelled_flights_SQL_9_CSV_FILE, index=False)
    sqlresult_10 = pd.read_sql_query(sqlEdw10, connection)
    sqlresult_10.to_csv(Yet_to_Depart_flights_SQL_10_CSV_FILE, index=False)
    sqlresult_11 = pd.read_sql_query(sqlEdw11, connection)
    sqlresult_11.to_csv(Residual_cancels_SQL_11_CSV_FILE, index=False)
    sqlresult_12 = pd.read_sql_query(sqlEdw12, connection)
    sqlresult_12.to_csv(REPOSITIONED_FLIGHTS_SQL_12_CSV_FILE, index=False)
    sqlresult_13 = pd.read_sql_query(sqlEdw13, connection)
    sqlresult_13.to_csv(REPOSITIONED_FLIGHTS_COUNT_SQL_13_CSV_FILE, index=False)
    sqlresult_14 = pd.read_sql_query(sqlEdw14, connection)
    sqlresult_14.to_csv(DIVERTED_FLIGHTS_SQL_14_CSV_FILE, index=False) 
    ...
    sqlresult_23 = pd.read_sql_query(sqlEdw23, connection)
    sqlresult_23.to_csv(UA_Seat_Cancels_CSV_FILE, index=False)
    sqlresult_24 = pd.read_sql_query(sqlEdw24, connection)
    sqlresult_24.to_csv(UA_SeatCnxl_2019_CSV_FILE, index=False)
    ...
    sqlMf4 = m4.read()
    m4.close()
    MFsqlresult_1 = pd.read_sql_query(sqlMf1,masflight_connection_string)
    MFsqlresult_1.to_csv(OA_SQL_1_CSV_FILE, index=False)
    MFsqlresult_2 = pd.read_sql_query(sqlMf2,masflight_connection_string)
    MFsqlresult_2.to_csv(OA_SQL_2_CSV_FILE, index=False)
    MFsqlresult_3 = pd.read_sql_query(sqlMf3,masflight_connection_string)
    MFsqlresult_3.to_csv(OA_Line_CSV_FILE, index=False)
    MFsqlresult_4 = pd.read_sql_query(sqlMf4,masflight_connection_string)
    MFsqlresult_4.to_csv(OA_SeatCnxl_CSV_FILE, index=False)
    ...
    wb.Application.Run("ClearSheet", "UA_SeatCnxl_2019")
    wb.Application.Run("ClearSheet", "OA_SeatCnxl")
    ...
    wb.Application.Run("ImportData", UA_SeatCnxl_2019_CSV_FILE, "UA_SeatCnxl_2019")
    wb.Application.Run("ImportData", OA_SeatCnxl_CSV_FILE, "OA_SeatCnxl")     
   ...
   # pngSys.close()
    ...
    print('Email sent')
    connection.execute("""INSERT INTO opfl_dept_db.email_reports_monitoring (report_time, report_name, email_success, rows_count, error_msg) values (CURRENT_TIMESTAMP,'745_UAX','Y',0,'None');""") 
    connection.commit()                   
    logging.info('row inserted in reports monitoring table')                        
    server.quit()
else:
    send_error_email(attachment =LOG_FILE_NAME, extra_subject='Report failed due to data unavailaibility') 
    connection.execute("""INSERT INTO opfl_dept_db.email_reports_monitoring (report_time, report_name, email_success, rows_count, error_msg) values (CURRENT_TIMESTAMP,'745_UAX','N',0,'data unavailaibility');""") 
    connection.commit()   
    print('Email sent')
    logging.info('row inserted in reports monitoring table')  
    connection.close()
