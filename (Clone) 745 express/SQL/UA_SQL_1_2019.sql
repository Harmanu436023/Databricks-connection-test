...
sum(case when r.ex_company_id <> ' ' and r.company_id not in ('bu', 'ei', 'af', 'vs') then 0
                  when   r.sch_cd='s' and r.cnxl_cd='c' and r.cnxl_reason_cd != 'nt'  and r.cnxl_reason_cd not like 'z%' and r.divert_reason_cd = ' ' and r.company_id not in ('bu', 'ei', 'af', 'vs') then 1 else 0 end ) as cnxl_flt
...
,Sum(case when ex_company_id = ' ' and r.CNXL_CD='C' and r.DIVERT_REASON_CD='  ' and left(r.CNXL_REASON_CD,1)<>'X' AND r.cnxl_reason_cd NOT IN ('NF') and  r.cnxl_reason_cd != 'nt' and r.cnxl_reason_cd not like 'z%' and  r.report_month = extract(month  From  current_date - 1)
...
and r.report_day between '1' and '31'
