SELECT COALESCE(o1.TIME_STAMP, NULL) AS TIME_STAMP
	...
	,COALESCE(o1.sched_flt_dateminus2, 0) AS sched_flt_dateminus2
	,COALESCE(o1.cnxl_flt_dateminus2, 0) AS cnxl_flt_dateminus2
	,COALESCE(i1.a0_flt_dateminus2, NULL) AS a0_flt_dateminus2
	,COALESCE(i1.a14_flt_dateminus2, NULL) AS a14_flt_dateminus2
	,COALESCE(o1.d0_flt_dateminus2_o, NULL) AS d0_flt_dateminus2_o
	,COALESCE(o1.d0_flt_dateminus2, NULL) AS d0_flt_dateminus2
...
FROM (
	SELECT CURRENT_TIMESTAMP(6) AS TIME_STAMP
		...
		,sum(CASE 
				WHEN r.ex_company_id <> ' '
					...
					AND r.report_dt = CURRENT_DATE - 1
					THEN 0
				WHEN r.report_dt = CURRENT_DATE - 1
					AND r.sch_cd = 's'
					AND r.cnxl_cd = 'c'
	...
	WHERE r.report_dt BETWEEN (CURRENT_DATE - 1 - extract(day FROM CURRENT_DATE - 1))
	...
	SELECT CURRENT_TIMESTAMP(6) AS TIME_STAMP
		...
		,SUM(CASE 
				WHEN r.sch_cd = 's'
					...
					THEN 1
				ELSE 0
				END) AS in_sched_flt_dateminus2
		,SUM(CASE 
				WHEN r.ex_company_id <> ' '
					AND r.ex_arrv_diff_minutes <= 0
					AND r.report_dt = CURRENT_DATE - 2
