...
YES = datetime.date.today() - datetime.timedelta(days=2)
...
SQL_FOLDER = os.getcwd() + r'/SQL_test'
...
EMAIL_ADDR_FILE_TEST = os.getcwd() + r'/emailAddr_test2.csv'

filename = os.getcwd() + r'/0745_UAX_WB_test.xlsm'
...
def send_error_email(attachment, extra_subject=''):
    email_list_to = ['navya.khanduja@united.com','apoorva.singh@united.com']

    sender_address = 'apoorva.singh@united.com'
...
SS_CNT = pd.read_sql('select count(*) as cnt from co_prod_vmdb.rtf_flt_leg_operation_ss where report_dt = current_date-2', connection)
...
if ss_flag == True:
   ...
   # pngSys.close()
    ...
    msg['Subject'] = "[UDH TEST]: 745 Express Report Reconciliation " + YEST
