SELECT 'UA' AS Carrier
	...
	,SUM(CASE 
			...
			WHEN r.cs_flg = 1
				...
				THEN 1
			WHEN r.ch_flg = 1
				AND c.ch_qualified_ok_delay_ind = 'q'
				THEN 1
	...
	,Sum(CASE 
			WHEN ex_company_id = ' '
				...
				THEN 1
			ELSE 0
			END) AS Controllable_Cancels_LW
	,SUM(CASE 
			WHEN r.sch_cd = 's'
				...
				THEN 1
			ELSE 0
			END) AS Scheduled_Flights_LWMinus1
	,sum(CASE 
			WHEN r.ex_company_id <> ' '
				AND r.company_id NOT IN (
					'bu'
