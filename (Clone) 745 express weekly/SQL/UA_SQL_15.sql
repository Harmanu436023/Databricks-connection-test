...
Sum(case when ex_company_id = ' ' and r.CNXL_CD='C' 
...
SUM(CASE when r.sch_cd = 's' 
         and r.report_dt between current_date-7 and current_date-1 
         THEN 1 ELSE 0 END) AS Scheduled_Flights_LW,

sum(case when r.ex_company_id <> ' ' 
    ...
    when r.sch_cd='s' 
         ...
         and r.company_id not in ('bu', 'ei', 'af', 'vs') 
         and r.report_dt between current_date-7 and current_date-1 
         then 1 else 0 end ) as cnxl_flt_LW,
...
SUM(case when r.ex_company_id <> ' ' 
         ...
         and r.report_dt between current_date-7 and current_date-1 then 1
    when r.cs_flg= 1 
    ...
    when r.dprt_sta_cd = r.sch_dprt_sta_cd 
         ...
         and r.dprt_diff_minutes <= 0 
         and r.report_dt between current_date-7 and current_date-1 
         then 1 else 0 end) as new_dep0_cnt_LW,
...
SUM(CASE when r.ex_company_id <> ' ' 
     ...
     when r.cnxl_cd = ' ' 
         ...
         and r.report_dt between current_date-7 and current_date-1 
         THEN 1 ELSE 0 END) AS arr0_cnt_LW,

SUM(CASE WHEN r.EQUIP_CONNECT_CD = 'H' 
         and r.report_dt between current_date-7 and current_date-1 
         THEN 1 ELSE 0 END) AS STAR_SCH_FLT_COUNT_LW,
...
SUM(CASE WHEN r.EQUIP_CONNECT_CD = 'H' 
     ...
     when r.qualified_bus_delay_ind = 'q'  
         and r.report_dt between current_date-7 and current_date-1 
         THEN 1 ELSE 0 END) AS STAR_D00_COUNT_LW,

Sum(case when ex_company_id = ' ' 
         and r.CNXL_CD='C' 
         and r.DIVERT_REASON_CD='  ' 
         and left(r.CNXL_REASON_CD,1)<>'X' 
         and r.cnxl_reason_cd <> 'nt' 
         and r.cnxl_reason_cd not like 'z%' 
         and r.report_dt between current_date-7 and current_date-1 
         then 1 else 0 end) as Controllable_Cancels_LW,

sum(case when r.sch_cd in ('s', 'u') 
         and r.sch_dprt_sta_cd=r.dprt_sta_cd 
         and r.act_dprt_dtmz <> '0001-01-01 00:00:00' 
         and r.report_dt between current_date-7 and current_date-1 
         then 1 
    when r.sch_cd in ('s') 
         and r.cnxl_reason_cd not in (' ', 'zd', 'zo', 'zf') 
         and r.divert_reason_cd=' '  
         and r.report_dt between current_date-7 and current_date-1 
         then 1 
    else 0 end) as dep_cnt_LW,
...
sum(case when r.sch_cd in ('s', 'u') 
    ...
    when r.sch_cd in ('s') 
         ...
         and r.report_dt between current_date-7 and current_date-1 
         then 1 
    else 0 end) as arr_cnt_LW

from co_prod_vmdb.rtf_flt_leg_operation_ss r 

left outer join co_prod_vmdb.rtf_flt_leg_conx_saver adj
on adj.report_dt = r.report_dt 
...
left outer join co_prod_vmdb.vw_rtf_flt_leg_cust_hold c
...
and r.report_dt between current_date - 14 and current_date - 1
